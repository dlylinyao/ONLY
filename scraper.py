...
def sentiment_analyse(article):
    ...
    preds = []
    batches = [
        article[i:i+model_max_len]
        for i in range(0, len(article), model_max_len)]
...
def scrape_yle_news():
    ...
    if all_news_data:
        ...
        filtered_news_data = [article for article in all_news_data if article['Passes_threshold'] == 1]
        ...
        filtered_out_news_data = [article for article in all_news_data if article['Passes_threshold'] == 0]
