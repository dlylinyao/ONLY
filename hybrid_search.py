...
def init_all_engines():
    global documents, tfidf_vectorizer, tfidf_matrix, bert_model, bert_embeddings
...
def search_hybrid(query, top_k=5, threshold=0.20, alpha=0.5):
    if bert_model is None or tfidf_vectorizer is None or not query:
        return []

    query_embedding = bert_model.encode([query])
    semantic_scores = np.dot(query_embedding, bert_embeddings.T)[0]
    ...
    query_vec = tfidf_vectorizer.transform([expanded_query_str])
    tfidf_scores = np.dot(query_vec, tfidf_matrix.T).toarray()[0]
    ...
    for idx in ranked_indices[:top_k]:
        ...
        print(f"[DEBUG] '{query}' | Hybrid Score: {score:.4f} (Semantic: {semantic_scores[idx]:.4f}, TFIDF: {tfidf_scores[idx]:.4f})")
...
def search(query, mode="hybrid", top_k=5, for_rag=False, threshold=0.20):
    if mode == "semantic":
        results = search_semantic(query, top_k)
    elif mode == "tfidf":
        results = search_tfidf(query, top_k)
    elif mode == "hybrid":
        results = search_hybrid(query, top_k, threshold=threshold)
    else:
        results = search_hybrid(query, top_k, threshold=threshold)
